        ...
        for parent in range(n_nodes):
            if -1 != children_left[parent]:
                for child in [children_left[parent],children_right[parent]]:
                    wavelets[child] = abs(values[parent][0][0] - values[child][0][0])*np.sqrt(nsamples[child])
...
def simulate(params): # Trains a random forest using a set of hyper-parameters params
    ...
    X = np.array(list(training.drop(columns="smoking").itertuples(index=False, name=None)))
    Y = np.array(training.smoking)
    ...
    predictions = clf.predict(np.array(list(test.drop(columns="smoking"]).itertuples(index=False, name=None))))
    realScores = test.smoking
    ...
    data = pd.DataFrame({"ntrees":ntrees,"nFeatures":[nFeatures],"max_depth":max_depth,"min_samples_leaf":min_samples_leaf,
                                      "accuracy":len(performance.loc[performance.pre==performance.score])/len(performance),
                                      "precision":len(performance.loc[performance.pre==True].loc[performance.score==True])/len(performance.loc[performance.pre==True]),
                                      "recall":len(performance.loc[performance.pre==True].loc[performance.score==True])/len(performance.loc[performance.score==True]),
                                      "alpha":getAlpha(clf)})
    data.to_csv(f"newResultsClass/{round(nFeatures*10)}_{ntrees}_{max_depth}_{min_samples_leaf}_{ntest}.csv")
...
if __name__ == '__main__':
    ...
    df.loc[df.oral=="Y","oral"] = 0
    df.loc[df.oral=="N","oral"] = 1
    df.loc[df.tartar=="Y","tartar"] = 0
    df.loc[df.tartar=="N","tartar"] = 1
