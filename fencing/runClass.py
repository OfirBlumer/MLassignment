...
def simulate(params): # Trains a random forest using a set of hyper-parameters params
    ...
    X = np.array(list(training.drop(columns=["won","score"]).itertuples(index=False, name=None)))
    Y = np.array(training["won"])
    ...
    predictions = clf.predict(np.array(list(test.drop(columns=["won","score"]]).itertuples(index=False, name=None))))
    realScores = test["won"]
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("organizedFencingData.csv")
