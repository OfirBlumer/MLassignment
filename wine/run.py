...
def simulate(params): # Trains a random forest using a set of hyper-parameters params
    ...
    X = np.array(list(training.drop(columns="quality").itertuples(index=False, name=None)))
    Y = np.array(training["quality"])
    ...
    predictions = clf.predict(np.array(list(test.drop(columns="quality").itertuples(index=False, name=None))))
    realScores = test["quality"]
