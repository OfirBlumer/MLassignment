...
def simulate(params): # Trains a random forest using a set of hyper-parameters params
    ...
    X = np.array(list(training.drop(columns="60").itertuples(index=False, name=None)))
    Y = np.array(training["60"])
    ...
    predictions = clf.predict(np.array(list(test.drop(columns="60"]).itertuples(index=False, name=None))))
    realScores = test["60"]
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("sonar.all-data.xls",names=[str(i) for i in range(61)])
    df.loc[df["60"]=="R","60"] = 0
    df.loc[df["60"]=="M","60"] = 1
